...
exports.getPref = function (key, aDefault) {
  ...
  const prefType = exports.getPref.branch.getPrefType(key)
...
exports.setDefaultPrefs = function () {
  ...
  for (const [key, val] in Iterator(PREFS)) {
    switch (typeof val) {
      case "boolean":
...
exports.PREF_OBSERVER = {
  observe: function(aSubject, aTopic, aData) {
    ...
    exports.prefChangeListeners.forEach(function(listener) {
      // listener is sometimes set to null disabled
      if (!listener) {
        return
      }
 ...
 *
 * @param localPrefBranch [nsIPrefBranch] :
 ...
 *
 * @param prefKey[string] : Preferences keys, will prepend PREF_BRANCH for you
 * @param node [object] : The DOM node to add/change attribute
 * @param attributeName [string] : The name is attribute to add/change
 * @param attributeValueFunc [function] : The function to return the value of attribute to be set, if undefined is returned, will remove the attribute
 */
exports.mapPrefToAttribute = function (prefKey, node, attributeName, attributeValueFunc) {
  ...
  const listener = function(aData) {
    ...
    const prefValue = exports.getPref(aData)
    const newValue = attributeValueFunc(prefValue)
  ...
  const newListenerIndex = exports.prefChangeListeners.push(listener) - 1 // Minus one here since `push` return new length
  unload(function() {
 ...
 * @param prefKey [string] : Preferences keys, will prepend PREF_BRANCH for you
 * @param node [object] : The DOM node to add/change attribute
 * @param attributeName [string] : The name is attribute to add/remove
 */
